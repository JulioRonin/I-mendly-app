...
export default function LoginScreen({ onLogin, navigate }: Props) {
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: '#EFEFEF' }}>
      ...
      {/* ── DARK HEADER ── */}
      <div style={{ background: '#0A0A0A', padding: '52px 24px 36px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: -60, right: -40, width: 200, height: 200, borderRadius: '50%', background: '#C1E8D5', opacity: 0.07, filter: 'blur(40px)', pointerEvents: 'none' }} />
        ...
        <span style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 700, fontSize: 11, color: '#C1E8D5', letterSpacing: '0.07em', textTransform: 'uppercase' }}>Inicio de sesión</span>
        <h1 style={{ fontFamily: 'Outfit, sans-serif', fontWeight: 900, fontSize: 42, color: 'white', letterSpacing: '-0.05em', lineHeight: 0.95, margin: '10px 0 0' }}>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar" style={{ padding: '0 20px 32px', marginTop: -20 }}>
        <div style={{ background: 'white', borderRadius: '28px 28px 24px 24px', padding: '28px 24px', boxShadow: '8px 8px 30px rgba(0,0,0,0.10), -4px -4px 16px rgba(255,255,255,0.9)' }}>
          ...
          {/* Role tabs */}
          <div className="flex gap-2 p-1.5 rounded-2xl" style={{ background: '#EFEFEF', marginBottom: 24 }}>
          ...
          <div style={{ marginBottom: 28, position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button onClick={() => setShowPass(!showPass)}
                style={{ position: 'absolute', right: 16, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#6A6A6A', padding: 0 }}>
                {showPass
            ...
            <div style={{ textAlign: 'right', marginTop: 8 }}>
              <span style={{ fontFamily: 'Outfit, sans-serif', fontSize: 12, fontWeight: 600, color: '#6A6A6A', cursor: 'pointer' }}>¿Olvidaste tu contraseña?</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3" style={{ margin: '22px 0' }}>
            ...
            <span style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, color: '#B0B0B0' }}>o continúa con</span>
            <div style={{ flex: 1, height: 1, background: 'rgba(0,0,0,0.07)' }} />
          </div>
          ...
          <div className="flex gap-3">
            {[
              ...
              <button key={s.name} style={{ flex: 1, padding: '13px', background: '#EFEFEF', border: 'none', borderRadius: 16, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '4px 4px 10px rgba(0,0,0,0.06), -2px -2px 8px rgba(255,255,255,0.8)', fontFamily: 'Outfit, sans-serif', fontWeight: 600, fontSize: 13, color: '#0A0A0A' }}>
                {s.icon} {s.name}
              </button>
            ))}
          </div>
          ...
          {/* Register link */}
          <p style={{ fontFamily: 'Inter, sans-serif', fontSize: 13, color: '#6A6A6A', textAlign: 'center', marginTop: 22 }}>
            ¿No tienes cuenta?{' '}
            <span onClick={() => navigate(AppView.PROVIDER_ONBOARDING)} style={{ color: '#0A0A0A', fontWeight: 700, cursor: 'pointer', textDecoration: 'underline', textDecorationColor: '#C1E8D5' }}>
              Regístrate gratis
            </span>
          </p>
        </div>
        ...
        <div className="flex justify-center gap-6" style={{ marginTop: 20 }}>
          {[
            { icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6A6A6A" strokeWidth="2" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>, text: 'Encriptación bancaria' },
            { icon: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#6A6A6A" strokeWidth="2" strokeLinecap="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>, text: 'Soporte 24/7' },
          ].map(b => (
            <div key={b.text} className="flex items-center gap-1.5">
              {b.icon}
              <span style={{ fontFamily: 'Outfit, sans-serif', fontSize: 11, fontWeight: 600, color: '#6A6A6A', letterSpacing: '0.02em' }}>{b.text}</span>
            </div>
          ))}
        </div>
      </div>
