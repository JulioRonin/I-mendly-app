  ...
  return (
    <div className="h-full flex flex-col" style={{ background: '#F5F5F5' }}>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar" style={{ padding: '0 20px 40px' }}>
        ...
        {/* Provider card */}
        <div style={{ background: '#1F1F1F', borderRadius: 22, padding: '18px', marginBottom: 14, position: 'relative', overflow: 'hidden' }}>
          <div style={{ position: 'absolute', top: -40, right: -30, width: 150, height: 150, borderRadius: '50%', background: '#3DB87A', opacity: 0.07, filter: 'blur(30px)', pointerEvents: 'none' }} />
          <div className="flex items-center gap-3" style={{ marginBottom: 12 }}>
            <div style={{ width: 52, height: 52, borderRadius: 18, background: order.providerAvatarColor, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Urbanist, sans-serif', fontWeight: 800, fontSize: 18, color: 'white', flexShrink: 0 }}>
              {order.providerInitials}
            </div>
            <div style={{ flex: 1 }}>
              <h2 style={{ fontFamily: 'Urbanist, sans-serif', fontWeight: 700, fontSize: 16, color: 'white', letterSpacing: '-0.02em', margin: '0 0 2px' }}>{order.providerName}</h2>
              <p style={{ fontFamily: 'Urbanist, sans-serif', fontSize: 13, color: 'rgba(255,255,255,0.4)', margin: 0 }}>{order.serviceName}</p>
            </div>
          ...
          <div style={{ background: 'rgba(193,232,213,0.12)', border: '1px solid rgba(193,232,213,0.2)', borderRadius: 14, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontFamily: 'Urbanist, sans-serif', fontWeight: 700, fontSize: 13, color: '#3DB87A' }}>Servicio en curso</span>
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: 20, padding: '16px', marginBottom: 14, boxShadow: '4px 4px 12px rgba(0,0,0,0.06), -2px -2px 8px rgba(255,255,255,0.9)', display: 'flex', gap: 14, alignItems: 'center' }}>
          <div style={{ width: 46, height: 46, borderRadius: 16, background: '#3DB87A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1F1F1F" strokeWidth="2" strokeLinecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
          </div>
          <div style={{ flex: 1 }}>
            <div className="flex items-center justify-between">
              <span style={{ fontFamily: 'Urbanist, sans-serif', fontWeight: 700, fontSize: 14, color: '#1F1F1F' }}>Pago en escrow</span>
            ...
            <p style={{ fontFamily: 'Urbanist, sans-serif', fontSize: 12, color: '#6B6B6B', margin: '4px 0 0', lineHeight: 1.5 }}>
        ...
        <div style={{ background: 'white', borderRadius: 20, padding: '18px', marginBottom: 14, boxShadow: '4px 4px 12px rgba(0,0,0,0.06), -2px -2px 8px rgba(255,255,255,0.9)' }}>
          <h3 style={{ fontFamily: 'Urbanist, sans-serif', fontWeight: 700, fontSize: 15, color: '#1F1F1F', letterSpacing: '-0.02em', marginBottom: 18 }}>Estado del servicio</h3>
          <div>
              ...
              <div key={i} style={{ display: 'flex', gap: 14 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 28, flexShrink: 0 }}>
                  <div style={{
                    ...
                    background: event.done ? '#3DB87A' : event.active ? '#1F1F1F' : '#F5F5F5',
                  ...
                  {i < order.timeline.length - 1 && (
                    <div style={{ width: 2, flex: 1, margin: '4px 0', background: event.done ? '#3DB87A' : 'rgba(0,0,0,0.07)', minHeight: 20 }} />
                  )}
                </div>
                {/* Content */}
                <div style={{ paddingBottom: i < order.timeline.length - 1 ? 20 : 0, paddingTop: 4 }}>
                  <p style={{ fontFamily: 'Urbanist, sans-serif', fontWeight: event.done || event.active ? 700 : 500, fontSize: 14, color: event.done ? '#1F1F1F' : event.active ? '#1F1F1F' : '#A8A8A8', margin: '0 0 2px' }}>
                    {event.label}
                  </p>
                  {event.timestamp && (
                    <p style={{ fontFamily: 'Urbanist, sans-serif', fontSize: 11, color: '#A8A8A8', margin: 0 }}>
        ...
        {order.status === 'pending_validation' && (
          <div style={{ background: 'white', borderRadius: 20, padding: '18px', boxShadow: '4px 4px 12px rgba(0,0,0,0.06), -2px -2px 8px rgba(255,255,255,0.9)', border: '2px solid #3DB87A' }}>
            <p style={{ fontFamily: 'Urbanist, sans-serif', fontWeight: 700, fontSize: 15, color: '#1F1F1F', marginBottom: 6 }}>¿El servicio quedó bien?</p>
            <p style={{ fontFamily: 'Urbanist, sans-serif', fontSize: 13, color: '#6B6B6B', lineHeight: 1.6, marginBottom: 16 }}>
